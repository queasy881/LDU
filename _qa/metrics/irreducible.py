...
from qapaths import ROOT, CORPUS, OUT, ensure_out

REPORT = os.path.join(ensure_out(OUT), "irred_report.csv")


def default_bin():
    """DS_REAL_BIN, else a corpus DLL this repo can build."""
    b = os.environ.get("DS_REAL_BIN")
    if b:
        return b
    for c in ("board.dll", "torture.dll", "math_ops.dll"):
        p = os.path.join(CORPUS, c)
        if os.path.exists(p):
            return p
    sys.exit("no test binary: set DS_REAL_BIN, or build _qa/fixtures/corpus/compile_all.bat")


def run_dump():
    """Re-dump every function with DS_IRRED_REPORT set."""
