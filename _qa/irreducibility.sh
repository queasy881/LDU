...
REPORT="$(pwd)/_qa/irred_report.csv"
rm -f "$REPORT"
...
# DS_REAL_BIN defaults to the NullWare binary the full gate uses.
: "${DS_REAL_BIN:=C:\\Users\\User\\Downloads\\NullWare\\NullWare\\NullWare\\build\\bin\\Release\\NullWare.dll}"
export DS_REAL_BIN
export DS_PAIRS_CAP="${DS_PAIRS_CAP:-2000}"
export DS_IRRED_REPORT="$REPORT"

rm -f _qa/pairs/fn_*.txt 2>/dev/null || true
