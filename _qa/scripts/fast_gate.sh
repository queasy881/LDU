...
export DS_PAIRS_CAP="${DS_PAIRS_CAP:-1500}"
export DS_PAIRS_DIR="$PAIRS"
EXE=$(qa_dump_exe)
[ -n "$EXE" ] || { echo "no dump_pairs exe; run _qa/scripts/rebuild.sh first" >&2; exit 2; }
mkdir -p "$PAIRS"; rm -f "$PAIRS"/fn_*.txt
"$EXE" --nocapture >/dev/null 2>&1
echo "DUMPED $(ls "$PAIRS"/fn_*.txt 2>/dev/null | wc -l)"
...
echo "GOTO_TOTAL $(grep -rho 'goto [A-Za-z0-9_]*;' "$PAIRS"/fn_*.txt 2>/dev/null | wc -l)   (<=700 fence; honest baseline 568; was 142 w/ invented flags)"
echo "INVENTED_FLAGS $(grep -l '__at_[0-9a-f]' "$PAIRS"/fn_*.txt 2>/dev/null | wc -l)   (must stay 0)"
echo "STATE_MACHINES $(grep -l 'switch (__state)' "$PAIRS"/fn_*.txt 2>/dev/null | wc -l)   (must stay 0)"
bash "$QA_SCRIPTS/verify_fast.sh" 2>&1 | tail -1
