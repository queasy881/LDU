...
# Usage: bash _qa/scripts/redump_par.sh [N]   (N workers, default 14)
...
mapfile -t SORTED < <(
  awk 'FNR==1{ b=0; if(match($0,/blocks=[0-9]+/)) b=substr($0,RSTART+7,RLENGTH-7);
      ...
      "$PAIRS"/fn_*.txt | sort -rn | awk '{print $2}')
TOTAL=${#SORTED[@]}
...
echo "PAR_REDUMP_EXIT=$fail  workers=$N  pairs=$(ls "$PAIRS"/fn_*.txt | wc -l)/$TOTAL"
